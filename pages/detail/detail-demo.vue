		...
		<view class="u-skeleton">
			...
			<template v-else>
				<video :src="playUrl" id="play" :title="$H.ellipsis(title,14)" controls show-casting-button
					...
					:unit-id="$H.getConfig('play_start_ad')" danmu-btn enable-danmu
					:danmu-list="danmuList" play-strategy="3">
					...
					<template v-if="isFullscreen && isDmInput">
						<view style="position: absolute;right: 0;left: 0;z-index: 99;height: 85rpx;"
							class="flex align-center bg-bai" :style="{bottom:dmInputH,width:dmInputW+'px'}">
			...
			<swiper :current="tabCurrent" @change="changeSwiper" :style="{height:swiperH}">
				...
				<swiper-item>
								...
								<template v-if="item.data.type == 'msg'">
									...
									<view class="flex align-start px-2 my-2" :id="'chat'+index"
										...
										<view class="flex flex-column">
											<view :style="item.data.content.length <= 3 ? 'text-align: center;' : 'text-align: left;'"
												:class="userInfo.avatarUrl == item.from_pic ? 'bg-lv' : 'bg-bai'"
												style="border-radius: 10rpx;min-width: 100rpx;max-width: 400rpx;" class="p-2 mx-2">
												{{item.data.content}}
											</view>
											<text class="font27 gray mx-2 tc"
												style="text-decoration: underline;"
												@click="toMsgCurrent(item.data.current,item.data.episodeCurrent)">
												{{episode[item.data.episodeCurrent].episode}} {{$H.s_to_hs(item.data.current)}}
											</text>
										</view>
									</view>
								</template>
					...
					<view style="height: 100rpx;margin-bottom: env(safe-area-inset-bottom);"
						class="border-top-hui bg-bai flex align-center abs-bottom" v-if="isAddRoom"
						:style="{bottom:inputH}">
